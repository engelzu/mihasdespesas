    ...
    const sidebarHTML = `
        ...
        <div id="global-sidebar" class="fixed top-0 left-0 bottom-0 w-64 bg-surface-container-lowest z-[1001] shadow-2xl transform -translate-x-full transition-transform duration-300 flex flex-col">
            ...
            <div class="flex-1 p-md flex flex-col gap-sm">
                ...
                <button onclick="forceAppUpdate()" class="flex items-center gap-md p-md rounded-lg hover:bg-primary-container/30 text-primary w-full text-left font-label-lg transition-colors">
            ...
            <div class="p-md bg-surface-container-low m-md rounded-xl border border-outline-variant/30">
                ...
                <p class="font-body-sm text-[11px] text-on-surface-variant mt-xs">Uso de memória do aparelho para salvar dados offline.</p>
            </div>
        </div>
    `;
        ...
        navigator.serviceWorker.register(swPath).then(registration => {
            ...
            function showUpdateApp() {
                showCustomModal(
                    ...
                    'Uma nova versão do aplicativo está disponível. Por favor, atualize seu app para continuar aproveitando os novos recursos.', 
                    'confirm', 
                    () => {
                        if (registration.waiting) {
                            registration.waiting.postMessage({ type: 'SKIP_WAITING' });
                ...
                // Mudar texto dos botões no modal
                setTimeout(() => {
                    const cancelBtn = document.getElementById('modal-btn-cancel');
                    const confirmBtn = document.getElementById('modal-btn-confirm');
                    if (cancelBtn) cancelBtn.innerText = 'Depois';
                    if (confirmBtn) confirmBtn.innerText = 'Atualizar App';
                }, 50);
            }
